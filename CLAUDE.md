...
12. slot 的 `auto` 字段(Claude 会话名)来自 `~/.claude/sessions/<claude进程pid>.json` 的 `name`,通过 /proc 祖先链定位(hook 是 claude 的子进程,祖先 pid 即文件名)。**必须校验文件里 `sessionId == sid`** 再采用(pid 会被复用,不校验会串到别人会话的名字);读不到就 sticky 用上一次的值,绝不为此报错。显示优先级固定:✎ alias > CCTL_NAME/.cctl-name > auto > 目录名·短id。
    **`nameSource:"derived"` 的 name 是 Claude 启动占位名**(目录+两位哈希,如 `opt-c7`),绝不能当 auto 采用;sticky 回填时旧 slot 里存的占位名同样要丢弃(和当前占位名相等即丢)。**没有真名(alias/named/auto 全空)时绝不写 tab 标题**——Claude Code ≥2.1 自己会往 tab 写话题标题(`✳ ...`),拿 folder 名去盖它就是"切走标题就回退"的 bug 根源。widget 侧每次轮询还会活读 `sessions/<cpid>.json` 的 name(同样校验 sessionId + 过滤 derived),这样 `/rename` 不用等下一个 hook 事件、2 秒内上屏。
...
14. **渲染必须原地更新,不许整树重建**:数据变化(状态/tokens/名字/行排序)只准 `config()` 改字/改色、`pack_forget()+pack()` 重排已有行;只有**会话增删、限额条增删、宽度/缩放变化**(struct 指纹)才允许 destroy 重建。每次变化 destroy 全部控件会让整个面板肉眼可见地闪一下,多会话状态频繁翻转时就是"一闪一闪"。行文案统一走 `_row_texts()`,build 和 update 共用,防止两条路径漂移。红绿灯圆点的 1s 闪烁提醒是**有意设计**(用户确认保留),别当 bug 修掉。
15. "后台任务中"判定:**后台 shell 跑完/没跑完不产生任何 hook 事件**,唯一信号是 sessions/<cpid>.json 的 `status` 字段(Claude 自己维护,busy/idle)。widget 每次轮询只对 slot 状态为 finished/idle 的行做**升级**(→ 运行中·后台任务中),绝不降级 needs_confirmation、绝不反向把 busy 当 finished;同样必须校验 sessionId;bg 行豁免 30min STALE 置灰(它每 2s 都被实时确认过)。
