...
3. widget 里点 ✎(或**双击行标题**)直接改——**只对当前这个会话生效**,会话结束即失效;改完会顺手把该会话的终端 tab 标题也刷成新名字

前两种是持久命名(跨会话生效,hex 短 id 自动隐藏);第三种是临时的会话级别名,显示时同样隐藏短 id。别名优先级最高,但**不会**波及同目录的其它/新会话——想要"这个目录以后都叫 X",用 `.cctl-name`。
